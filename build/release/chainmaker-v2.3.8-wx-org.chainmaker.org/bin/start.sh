...
config_file="../config/wx-org.chainmaker.org/chainmaker.yml"
...
function start_vm_go() {
  container_name=VM-GO-wx-org.chainmaker.org-name-20260119111332
...
function start_docker_vm_go() {
  container_name=DOCKERVM-wx-org.chainmaker.org-name-20260119111332
  #check container exists
  exist=$(docker ps -f name="$container_name" --format '{{.Names}}')
  if [ "$exist" ]; then
    echo "$container_name already RUNNING, please stop it first."
...
pid=$(ps -ef | grep chainmaker | grep "\-c ../config/wx-org.chainmaker.org/chainmaker.yml name-20260119111332" | grep -v grep |  awk  '{print $2}')
if [ -z "${pid}" ];then
    ...
    # start chainmaker
    #nohup ./chainmaker start -c ../config/wx-org.chainmaker.org/chainmaker.yml name-20260119111332 > /dev/null 2>&1 &
    nohup ./chainmaker start -c ../config/wx-org.chainmaker.org/chainmaker.yml name-20260119111332 >> panic.log 2>&1 &
