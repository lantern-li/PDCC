...
function check_validity(){
  ...
  echo "################ 日志预处理中 ################"
  # 缩减日志规模
  startString="commit block \[$((BEGIN_HEIGHT-1))"
  endString="attempt enter new height to ($((END_HEIGHT+1)))"
...
function check_block(){
  height=${1}
  # 提交节点是否参与共识和提交区块
  if [ $(grep "attempt enter new height to ($i)" system.log.* | wc -l) != $NODE_NUM ]; then
    echo "某节点未参与共识 $i"
    exit 1
  fi
  if [ $(grep "commit block \[$i" system.log.* | wc -l) != $NODE_NUM ]; then
    echo "某节点未提交区块 $i"
    exit 1
  fi
  echo "-------- 区块: "$height" 日志有效 --------"
   ...
   if [ `grep -c "$is_proposer" $grepped_log_path` -ne '0' ];then
     ...
     do
       if [[ $line =~ $fetch_txs_flag ]]; then
         ...
         str=$"Store异步落库":${line#*$put_block_flag}
