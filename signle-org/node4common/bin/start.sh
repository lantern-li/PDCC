...
config_file="../config/node4common/chainmaker.yml"
...
function start_vm_go() {
  container_name=VM-GO-wx-org.chainmaker.org
  ...
  then
    ...
    --name VM-GO-wx-org.chainmaker.org \
...
function start_docker_vm_go() {
  container_name=DOCKERVM-wx-org.chainmaker.org
  #check container exists
  exist=$(docker ps -f name="$container_name" --format '{{.Names}}')
  if [ "$exist" ]; then
    echo "$container_name already RUNNING, please stop it first."
  ...
  docker run -itd \
    ...
    --name DOCKERVM-wx-org.chainmaker.org \
...
pid=$(ps -ef | grep chainmaker | grep "\-c ../config/node4common/chainmaker.yml" | grep -v grep |  awk  '{print $2}')
if [ -z "${pid}" ];then
    ...
    # start chainmaker
    #nohup ./chainmaker start -c ../config/wx-org4.chainmaker.org/chainmaker.yml > /dev/null 2>&1 &
    nohup ./chainmaker start -c ../config/node4common/chainmaker.yml > panic.log 2>&1 &
