...
config_file="../config/node2/chainmaker.yml"
...
pid=$(ps -ef | grep chainmaker | grep "\-c ../config/node2/chainmaker.yml" | grep -v grep |  awk  '{print $2}')
if [ -z "${pid}" ];then
    ...
    # start chainmaker
    #nohup ./chainmaker start -c ../config/wx-org2.chainmaker.org/chainmaker.yml > /dev/null 2>&1 &
    nohup ./chainmaker start -c ../config/node2/chainmaker.yml > panic.log 2>&1 &
