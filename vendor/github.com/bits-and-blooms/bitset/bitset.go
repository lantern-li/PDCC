...
func init() {
	isLittleEndian = func() bool {
		...
		return *(*byte)(unsafe.Pointer(&u32)) == 0x04
	}()
}
...
// SetAtomic atomically sets the bit at position i to 1 using atomic operations.
// This method is thread-safe and can be used in concurrent environments.
// Returns the modified BitSet to allow for method chaining.
func (b *BitSet) SetAtomic(i uint) *BitSet {
	if i >= b.length { // if we need more bits, make 'em
		b.extendSet(i)
	}
	b8Set := (*[8]uint8)(unsafe.Pointer(&b.set[i>>log2WordSize]))
	idx := wordsIndex(i)
	if isLittleEndian {
		Or8(&b8Set[idx>>3], 1<<(idx&7))
	} else {
		Or8(&b8Set[7-idx>>3], 1<<(idx&7))
	}
