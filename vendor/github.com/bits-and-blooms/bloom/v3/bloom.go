...
func FromWithM(data []uint64, m, k uint) *BloomFilter {
	return &BloomFilter{m, k, bitset.FromWithLength(m, data)}
}
...
// AddAtomic adds data to the Bloom Filter in a thread-safe manner.
// Returns the filter (allows chaining)
func (f *BloomFilter) AddAtomic(data []byte) *BloomFilter {
	h := baseHashes(data)
	for i := uint(0); i < f.k; i++ {
		f.b.SetAtomic(f.location(h, i))
	}
	return f
}

// AddStringAtomic adds a string to the Bloom Filter in a thread-safe manner.
// Returns the filter (allows chaining)
func (f *BloomFilter) AddStringAtomic(data string) *BloomFilter {
	return f.AddAtomic([]byte(data))
}
