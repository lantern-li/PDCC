...
// SignBlockWithHash sign the block with hash (in fact, here we sign block hash...) with signing member
// return signature bytes
func SignBlockWithHash(hashType string, singer protocol.SigningMember, blockHash []byte) ([]byte, error) {
	if singer == nil {
		return nil, fmt.Errorf("sign block signer == nil")
	}
