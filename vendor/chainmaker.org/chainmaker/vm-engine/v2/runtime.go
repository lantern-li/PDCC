...
func (r *RuntimeInstance) Invoke(
	...
	for {
		...
		case recvMsg := <-r.sandboxMsgCh:
			...
			case protogo.DockerVMType_ENCRYPT_REQUEST:
				r.logger.DebugDynamic(func() string {
					return fmt.Sprintf("tx [%s] start encrypt", uniqueTxKey)
				})
				var encryptResponse *protogo.DockerVMMessage
				encryptResponse, gasUsed = r.handleEncryptRequest(uniqueTxKey, recvMsg, txSimContext, gasUsed)
				r.sendSysResponse(encryptResponse)
				r.logger.DebugDynamic(func() string {
					return fmt.Sprintf("tx [%s] finish encrypt", uniqueTxKey)
				})

			case protogo.DockerVMType_DECRYPT_REQUEST:
				r.logger.DebugDynamic(func() string {
					return fmt.Sprintf("tx [%s] start decrypt", uniqueTxKey)
				})
				var decryptResponse *protogo.DockerVMMessage
				decryptResponse, gasUsed = r.handleDecryptRequest(uniqueTxKey, recvMsg, txSimContext, gasUsed)
				r.sendSysResponse(decryptResponse)
				r.logger.DebugDynamic(func() string {
					return fmt.Sprintf("tx [%s] finish decrypt", uniqueTxKey)
				})
			...
			case protogo.DockerVMType_CREATE_KV_ITERATOR_REQUEST:
				...
				specialTxType = protocol.ExecOrderTxTypeSpecial
