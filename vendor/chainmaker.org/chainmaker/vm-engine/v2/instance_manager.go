...
func NewInstancesManager(
	...
	if dockerVMConfig.KMSConfig.Enable {
		if kmsProviders == nil {
			msg := "fail to init docker manager, kmsProviders map is nil"
			logger.Errorf(msg)
			return nil, errors.New(msg)
		}
		kmsProvider, exist := kmsProviders[dockerVMConfig.KMSConfig.Id]
		if !exist {
			msg := fmt.Sprintf("fail to init docker manager, "+
				"not exist kms provider with kms id: [%v]", dockerVMConfig.KMSConfig.Id)
			logger.Errorf(msg)
			return nil, errors.New(msg)
		}
		newDockerManager.kmsProvider = kmsProvider
	}
...
func (m *InstancesManager) StopVM() error {
	...
	m.mgrLogger.Infof("chain [%s] docker vm stop success :)", m.chainId)
	return nil
}
