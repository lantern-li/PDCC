...
func (r *RuntimeInstance) handlerCallContract(
	...
	contractName, contractMethod, callContractReq, checked := r.baseChechForCrossCall(recvMsg, response)
	if !checked {
	...
	// above v241, merge the write sets before cross calling,
	// for that the child tx can access the newest data updated by the father tx
	if txSimContext.GetBlockVersion() >= version238 {
		writeMapBytes := recvMsg.SysCallMessage.Payload[config.KeyWriteMap]
		if len(writeMapBytes) != 0 {
			writeMap := make(map[string][]byte)
			if err = json.Unmarshal(writeMapBytes, &writeMap); err != nil {
				errMsg := fmt.Sprintf("[call contract] failed to get wirteMap for [%s], err: %s",
					contractName, err.Error())
				r.setError(errMsg, response)
				return response, gasUsed, specialTxType
			}

			if gasUsed, err = r.mergeSimContextWriteMap(txSimContext, writeMap, gasUsed, contractName); err != nil {
				errMsg := fmt.Sprintf("[call contract] failed to merge the sim context write map for [%s], err: %s",
					contractName, err.Error())
				r.setError(errMsg, response)
				return response, gasUsed, specialTxType
			}
...
func (r *RuntimeInstance) baseChechForCrossCall(recvMsg *protogo.DockerVMMessage,
	response *protogo.DockerVMMessage) (string, string, *protogo.CallContractRequest, bool) {
	callContractPayload := recvMsg.SysCallMessage.Payload[config.KeyCallContractReq]
	callContractReq := new(protogo.CallContractRequest)
	...
	return contractName, contractMethod, callContractReq, true
}
...
func (r *RuntimeInstance) handleEncryptRequest(txId string, recvMsg *protogo.DockerVMMessage,
	txSimContext protocol.TxSimContext, gasUsed uint64) (*protogo.DockerVMMessage, uint64) {
	response := r.newEmptyResponse(txId, protogo.DockerVMType_ENCRYPT_RESPONSE)
	encryptKey := recvMsg.SysCallMessage.Payload[config.KeyEncryptDataKey]

	if r.DockerManager == nil || r.DockerManager.kmsProvider == nil {
		msg := fmt.Sprintf("[%v] handle encrypt error, unexpected nil", txId)
		r.logger.Errorf(msg)
		response.SysCallMessage.Message = msg
		response.SysCallMessage.Code = protocol.ContractSdkSignalResultFail
		return response, gasUsed
	}
	value, err := r.DockerManager.kmsProvider.Encrypt(encryptKey)
	if err != nil {
		r.logger.Errorf("[%v] handle encrypt error: %v", txId, err)
		response.SysCallMessage.Message = err.Error()
		response.SysCallMessage.Code = protocol.ContractSdkSignalResultFail
		return response, gasUsed
	}
	...
	response.SysCallMessage.Payload = map[string][]byte{
		config.KeyEncryptDataValue: value,
	}

	blockVersion := txSimContext.GetBlockVersion()
	gasConfig := gasutils.NewGasConfig(txSimContext.GetLastChainConfig().AccountConfig)
	gasUsed, err = gas.GetEncryptGasUsed(blockVersion, gasConfig, gasUsed, []byte{}, []byte{}, value)
	if err != nil {
		r.logger.Errorf("[%v] get encrypt gas used error: %s", txId, err)
		response.SysCallMessage.Message = err.Error()
		response.SysCallMessage.Code = protocol.ContractSdkSignalResultFail
		return response, gasUsed
	}
...
func (r *RuntimeInstance) handleDecryptRequest(txId string, recvMsg *protogo.DockerVMMessage,
	txSimContext protocol.TxSimContext, gasUsed uint64) (*protogo.DockerVMMessage, uint64) {
	response := r.newEmptyResponse(txId, protogo.DockerVMType_DECRYPT_RESPONSE)
	decryptKey := recvMsg.SysCallMessage.Payload[config.KeyDecryptDataKey]

	if r.DockerManager == nil || r.DockerManager.kmsProvider == nil {
		msg := fmt.Sprintf("[%v] handle decrypt error, unexpected nil", txId)
		r.logger.Errorf(msg)
		response.SysCallMessage.Message = msg
		response.SysCallMessage.Code = protocol.ContractSdkSignalResultFail
		return response, gasUsed
	}
	value, err := r.DockerManager.kmsProvider.Decrypt(decryptKey)
	if err != nil {
		r.logger.Errorf("[%v] handle decrypt error: %v", txId, err)
		response.SysCallMessage.Message = err.Error()
		response.SysCallMessage.Code = protocol.ContractSdkSignalResultFail
		return response, gasUsed
	}
	response.SysCallMessage.Code = protocol.ContractSdkSignalResultSuccess
	response.SysCallMessage.Payload = map[string][]byte{
		config.KeyDecryptDataValue: value,
	}

	blockVersion := txSimContext.GetBlockVersion()
	gasConfig := gasutils.NewGasConfig(txSimContext.GetLastChainConfig().AccountConfig)
	gasUsed, err = gas.GetDecryptGasUsed(blockVersion, gasConfig, gasUsed, []byte{}, []byte{}, value)
	if err != nil {
		r.logger.Errorf("[%v] get decrypt gas used error: %s", txId, err)
		response.SysCallMessage.Message = err.Error()
		response.SysCallMessage.Code = protocol.ContractSdkSignalResultFail
		return response, gasUsed
	}
