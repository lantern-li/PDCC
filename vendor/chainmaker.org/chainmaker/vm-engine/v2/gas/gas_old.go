...
const (
	...
	EncryptGasPrice uint64 = 1

	// DecryptGasPrice decrypt gas used
	DecryptGasPrice uint64 = 1
)

// GetEncryptGasUsedLt2312 returns encrypt gas used
func GetEncryptGasUsedLt2312(gasUsed uint64, value []byte) (uint64, error) {
	gasUsed += uint64(len(value)) * EncryptGasPrice
...
// GetDecryptGasUsedLt2312 returns decrypt gas used
func GetDecryptGasUsedLt2312(gasUsed uint64, value []byte) (uint64, error) {
	gasUsed += uint64(len(value)) * DecryptGasPrice
	if CheckGasLimit(gasUsed) {
		return 0, errors.New("over gas limited ")
	}
	return gasUsed, nil
