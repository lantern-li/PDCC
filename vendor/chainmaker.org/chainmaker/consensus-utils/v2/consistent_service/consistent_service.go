...
// GetNodeStatus 获取本地维护的节点状态
func (e *StatusConsistentEngine) GetNodeStatus(id string) (Node, error) {
	if id == "" {
		return nil, ErrorInvalidParameter
	}
	e.log.Debugf("GetNodeStatus:%s", id)
	...
	if _, ok := e.remoters[id]; ok {
		return e.remoters[id], nil
	}
	return nil, ErrorRemoterNotExist
}
