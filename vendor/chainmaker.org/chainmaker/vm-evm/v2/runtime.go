...
var (
	// 使用映射来存储不同chainId对应的logger实例
	evmLoggers = make(map[string]protocol.Logger)
	// 使用互斥锁确保并发安全
	evmLogMu sync.RWMutex
)

// getEvmLogInstance 获取指定chainId的Logger实例
func getEvmLogInstance(chainId string) protocol.Logger {
	// 加入读锁避免读写冲突
	evmLogMu.RLock()
	evmLogger, exists := evmLoggers[chainId]
	evmLogMu.RUnlock()
	...
	// 加写锁创建新实例
	evmLogMu.Lock()
	defer evmLogMu.Unlock()

	// 检查logger是否存在
	if logger, exists := evmLoggers[chainId]; exists {
		return logger
	}

	newLogger := logger.GetLoggerByChain(logger.MODULE_EVM, chainId)
	evmLoggers[chainId] = newLogger
	return newLogger
}
...
func (*InstancesManager) NewRuntimeInstance(txSimContext protocol.TxSimContext, chainId, method string, codePath string,
	...
	return &RuntimeInstance{
		...
		EvmLog:        getEvmLogInstance(chainId),
