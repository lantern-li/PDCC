...
const (
	// Proposer schedules transactions and generates DAG using random scheduling algorithm; consensus nodes execute transactions according to DAG order
	ProcessType_EXECUTE_ON_PROPOSE ProcessType = 0
	//  All consensus nodes schedule and execute transactions in parallel after propose using deterministic scheduling algorithm
	ProcessType_EXECUTE_AFTER_PROPOSE ProcessType = 1
)

var ProcessType_name = map[int32]string{
	0: "EXECUTE_ON_PROPOSE",
	1: "EXECUTE_AFTER_PROPOSE",
}

var ProcessType_value = map[string]int32{
	"EXECUTE_ON_PROPOSE":    0,
	"EXECUTE_AFTER_PROPOSE": 1,
}
...
const (
	// Random scheduling: Optimistic parallel execution (non-deterministic)
	AlgorithmType_RANDOM AlgorithmType = 0
	// Serial scheduling: Sequential execution (deterministic)
	AlgorithmType_SERIAL AlgorithmType = 1
	// Reorder scheduling: DAG-based deterministic reordering (deterministic parallel)
	AlgorithmType_REORDER AlgorithmType = 2
	// Wria scheduling: Batch-based for parallel execution(deterministic parallel)
	AlgorithmType_WRIA AlgorithmType = 3
	// BlockSTM scheduling: MVCC-based speculative parallel execution (deterministic parallel)
	AlgorithmType_BLOCKSTM AlgorithmType = 4
	// Graph scheduling: Graph-based deterministic parallel execution
	AlgorithmType_GRAPH AlgorithmType = 5
	// Aria scheduling: Deterministic batch-based execution
	AlgorithmType_ARIA AlgorithmType = 6
)
...
type SchedulerConfig struct {
	...
	// Process type:  transaction Process mode
	// DAG: Proposer schedules transactions and generates DAG using random scheduling algorithm; consensus nodes execute transactions according to DAG order (DAG-based Order-Execute)
	// DETERMINISTIC: All consensus nodes schedule and execute transactions in parallel using deterministic scheduling algorithm (Deterministic Order-Execute)
	ProcessType ProcessType `protobuf:"varint,2,opt,name=process_type,json=processType,proto3,enum=config.ProcessType" json:"process_type,omitempty"`
	// Scheduling algorithm: determines how transactions are ordered and executed
	// RANDOM: Optimistic parallel execution using random-generated DAG (non-deterministic, highest performance)
	// SERIAL: Sequential execution in deterministic order (fully deterministic, lower performance)
	// REORDER: DAG-based deterministic reordering for parallel execution (deterministic with better performance)
	AlgorithmType AlgorithmType `protobuf:"varint,3,opt,name=algorithm_type,json=algorithmType,proto3,enum=config.AlgorithmType" json:"algorithm_type,omitempty"`
	// Batch size for WRIA scheduler (number of transactions to process in each batch)
	// Default: 0 (will use runtime.NumCPU() * 10 if not set)
...
func (m *SchedulerConfig) GetProcessType() ProcessType {
	if m != nil {
		return m.ProcessType
	}
	return ProcessType_EXECUTE_ON_PROPOSE
}

func (m *SchedulerConfig) GetAlgorithmType() AlgorithmType {
	if m != nil {
		return m.AlgorithmType
	}
	return AlgorithmType_RANDOM
}

func (m *SchedulerConfig) GetBatchSize() uint32 {
	if m != nil {
		return m.BatchSize
	}
	return 0
...
func init() {
	proto.RegisterEnum("config.ProcessType", ProcessType_name, ProcessType_value)
	proto.RegisterEnum("config.AlgorithmType", AlgorithmType_name, AlgorithmType_value)
...
var fileDescriptor_2a46c6a6ad69c864 = []byte{
	...
	,
}
...
func (m *SchedulerConfig) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	_ = l
	if m.BatchSize != 0 {
		i = encodeVarintChainConfig(dAtA, i, uint64(m.BatchSize))
		i--
		dAtA[i] = 0x20
	}
	if m.AlgorithmType != 0 {
		i = encodeVarintChainConfig(dAtA, i, uint64(m.AlgorithmType))
		i--
		dAtA[i] = 0x18
	}
	if m.ProcessType != 0 {
		i = encodeVarintChainConfig(dAtA, i, uint64(m.ProcessType))
		i--
		dAtA[i] = 0x10
	}
	if m.EnableEvidence {
...
func (m *SchedulerConfig) Size() (n int) {
	...
	if m.ProcessType != 0 {
		n += 1 + sovChainConfig(uint64(m.ProcessType))
	}
	if m.AlgorithmType != 0 {
		n += 1 + sovChainConfig(uint64(m.AlgorithmType))
	}
	if m.BatchSize != 0 {
		n += 1 + sovChainConfig(uint64(m.BatchSize))
	}
	return n
}
	...
	for iNdEx < l {
		...
		case 3:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.AlgorithmType |= AlgorithmType(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
