...
type Stat struct {
	...
	// Time and cost for each filter review
	FiltersCosts []int64 `protobuf:"varint,4,rep,packed,name=filters_costs,json=filtersCosts,proto3" json:"filters_costs,omitempty"`
}
...
func (m *Stat) GetFiltersCosts() []int64 {
	if m != nil {
		return m.FiltersCosts
	}
	return nil
}
...
var fileDescriptor_6dcec2a32104aa33 = []byte{
	...
	,
}
...
func (m *Stat) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	var l int
	_ = l
	if len(m.FiltersCosts) > 0 {
		dAtA2 := make([]byte, len(m.FiltersCosts)*10)
		var j1 int
		for _, num1 := range m.FiltersCosts {
			num := uint64(num1)
			for num >= 1<<7 {
		...
		i = encodeVarintTxfilter(dAtA, i, uint64(j1))
		i--
		dAtA[i] = 0x22
	}
	if m.DbCosts != 0 {
...
func (m *Stat) Size() (n int) {
	...
	if len(m.FiltersCosts) > 0 {
		l = 0
		for _, e := range m.FiltersCosts {
			l += sovTxfilter(uint64(e))
		}
		n += 1 + sovTxfilter(uint64(l)) + l
	}
	return n
}
	...
	for iNdEx < l {
		...
		case 4:
			if wireType == 0 {
				...
				m.FiltersCosts = append(m.FiltersCosts, v)
			} else if wireType == 2 {
				var packedLen int
				for shift := uint(0); ; shift += 7 {
					if shift >= 64 {
				...
				elementCount = count
				if elementCount != 0 && len(m.FiltersCosts) == 0 {
					m.FiltersCosts = make([]int64, 0, elementCount)
				}
				for iNdEx < postIndex {
					var v int64
					for shift := uint(0); ; shift += 7 {
				...
				return fmt.Errorf("proto: wrong wireType = %d for field FiltersCosts", wireType)
			}
		default:
			iNdEx = preIndex
