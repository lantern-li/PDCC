...
func (s *StateKvDB) writeBatch(blockHeight uint64, batch protocol.StoreBatcher) error {
	...
	if s.dbHandle.GetDbType() == leveldbprovider.DbType_Leveldb {
		return s.writeBatchForLevelDB(blockHeight, batch) // 税总如果用levelDB，则继续走批量写逻辑，如果是sql，由于重庆出现的tdsql批量写可能导致的数据异常的问题，统一改为逐个写
	}
	...
	for k, v := range batch.KVs() {
		s.logger.Debugf("start put by dbhandle writeBatch,key:%s, valLen:%d",
			k, len(v))
		var (
			key = []byte(k)
			err error
		...
		if v == nil || len(v) == 0 {
			if err = s.dbHandle.Delete(key); errors.Is(err, rawsqlprovider.ErrNoRowExist) {
				err = nil
			}
		} else {
			err = s.dbHandle.Put(key, v)
		}

		if err != nil {
			s.logger.Errorf("writing state db for key[%s], error: %s", k, err)
			panic(err)
		}
	}
	...
	s.logger.Infof("write state db, block[%d], time used: (batchSplit:%d, total:%d)",
		blockHeight, len(batch.KVs()), time.Since(start).Milliseconds())
...
func (s *StateKvDB) writeBatchForLevelDB(blockHeight uint64, batch protocol.StoreBatcher) error {
	start := time.Now()
