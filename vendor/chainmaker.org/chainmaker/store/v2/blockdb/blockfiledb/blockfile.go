...
func (l *BlockFile) pushCache(path string, segFile *tbf.LockableFile) {
	...
	_, _, _, v, evicted := l.openedFileCache.SetEvicted(path, segFile)
	if evicted {
		// nolint
		if v == nil {
			return
...
func (l *BlockFile) openReadFile(path string) (*tbf.LockableFile, error) {
	...
	rfile, err = os.OpenFile(path, os.O_RDONLY, 0o666)
	if err != nil {
		return nil, err
	}
...
func (l *BlockFile) doReadFileSection(fiIndex *storePb.StoreInfo, timeOut time.Duration) ([]byte, error) {
	...
	if err != nil {
		...
		if lastFile {
			...
			readingTmpFile = true
			if lfile.Rfile, err = os.OpenFile(pat, os.O_RDONLY, 0o666); err != nil {
				return nil, err
			}
