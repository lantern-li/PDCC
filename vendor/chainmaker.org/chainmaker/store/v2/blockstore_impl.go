...
import (
	...
	"chainmaker.org/chainmaker/store/v2/statedb/bloom"
	...
	"chainmaker.org/chainmaker/store/v2/utils/wait"
...
func NewBlockStoreImpl(chainId string,
	...
	blockStore.waitG.Join(blockStore.WriteBatchFromChanToDB)
...
func (bs *BlockStoreImpl) WriteBatchFromChanToDB() {
	writedb := func(blockWithSerializedInfo *serialization.BlockWithSerializedInfo) {
		start := time.Now()
		block := blockWithSerializedInfo.Block
	...
	for {
		select {
		case <-bs.stopCh:
			bs.logger.Infof("block store stop writing batch from chan to db")
			return
		case wBatch := <-bs.writeBatchChan:
			writedb(wBatch.blockWithSerializedInfo)
		}
	}
}
...
func (bs *BlockStoreImpl) GetBlockHeaderByHeight(height uint64) (*commonPb.BlockHeader, error) {
	if bs.blockFileDB != nil && bs.storeConfig.BlockDbConfig.IsKVDB() {
		if bs.lastBlock != nil {
			...
			lastHeight := block.GetHeader().GetBlockHeight()
			if lastHeight < height {
				bs.logger.Warnf("block[%d] not found, lastest height is[%d]",
					height, lastHeight)
				return nil, nil
			}
			if lastHeight == height {
...
func (bs *BlockStoreImpl) GetTxWithRWSet(txId string) (*commonPb.TransactionWithRWSet, error) {
	...
	if bs.storeConfig.DisableResultDB {
		txInfo, err := bs.getTxInfoWithoutResultDB(txId)
		if err != nil {
			return nil, err
		}
		if txInfo == nil {
...
func (bs *BlockStoreImpl) GetTxInfoWithRWSet(txId string) (*commonPb.TransactionInfoWithRWSet, error) {
	if bs.storeConfig.DisableResultDB {
		return bs.getTxInfoWithoutResultDB(txId)
	}
...
func (bs *BlockStoreImpl) getTxInfoWithoutResultDB(txId string) (*commonPb.TransactionInfoWithRWSet, error) {
	...
	if !bs.storeConfig.DisableBlockFileDb {
		var blockWithRWSet *storePb.BlockWithRWSet
		blockWithRWSet, err = bs.GetBlockWithRWSets(txInfo.BlockHeight)
		if err != nil || blockWithRWSet == nil {
			return nil, err
		}
		block, rwSets = blockWithRWSet.Block, blockWithRWSet.TxRWSets
	} else {
		// if disable resultdb and block file db , we cant get rwset.
		block, err = bs.GetBlock(txInfo.BlockHeight)
		if err != nil || block == nil {
			return nil, err
		}
	}

	tx := &commonPb.TransactionInfoWithRWSet{
		Transaction:    block.GetTxs()[txInfo.TxIndex],
		BlockHeight:    txInfo.BlockHeight,
		BlockHash:      txInfo.BlockHash,
		TxIndex:        txInfo.TxIndex,
		BlockTimestamp: txInfo.BlockTimestamp,
	...
	if rwSets != nil {
		tx.RwSet = rwSets[txInfo.TxIndex]
	}
	return tx, nil
}
...
func (bs *BlockStoreImpl) GetHistoryForKey(contractName string, key []byte) (protocol.KeyHistoryIterator, error) {
	...
	return types.NewHistoryIterator(contractName, key, txs, bs.GetTxRWSet, bs.blockDB), nil
}
...
func (bs *BlockStoreImpl) GetBlockWithRWSets(height uint64) (*storePb.BlockWithRWSet, error) {
	if !bs.storeConfig.DisableBlockFileDb {
		...
		if len(data) == 0 {
			...
			data, err = bs.blockFileDB.ReadFileSection(index, 0)
			if err != nil {
				bs.logger.Warnf("get block[%d] with rwset from file failed: %v", height, err)
				return nil, err
			}
		}
...
func (bs *BlockStoreImpl) recoverResultDB(currentHeight uint64, savePoint uint64) error {
	height := bs.calculateRecoverHeight(currentHeight, savePoint)
	for ; height <= savePoint; height++ {
		bs.logger.Infof("[resultDB] recommitting lost blocks, blockNum=%d, lastBlockNum=%d", height, savePoint)
		blockWithSerializedInfo, err := bs.getBlockFromLog(height)
		if err != nil {
			return err
		}
