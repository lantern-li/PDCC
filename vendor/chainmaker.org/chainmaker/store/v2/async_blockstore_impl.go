...
// GetBlockHeaderByHeight returns a block header given its block height, or returns nil if none exists.
func (async *AsyncBlockStoreImpl) GetBlockHeaderByHeight(height uint64) (*common.BlockHeader, error) {
	blockInfo, ok := async.blocks.Load(height)
	if ok {
		if blk, isOk := blockInfo.(*commonPb.BlockInfo); isOk {
	...
	return async.BlockchainStore.GetBlockHeaderByHeight(height)
}
