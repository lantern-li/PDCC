...
type Wacsi interface {
	...
	ContractEncrypt(requestBody []byte, kmsProvider KMSProvider, memory []byte,
		data []byte, isLen bool) ([]byte, error)
	ContractDecrypt(requestBody []byte, kmsProvider KMSProvider, memory []byte,
		data []byte, isLen bool) ([]byte, error)
}
...
type TxSimContext interface {
	...
	// GetWithLock key from cache, record this operation to read set with lock
	GetWithLock(contractName, lockerName string, key []byte) ([]byte, error)
	// GetKeys key from cache, record this operation to read set
	GetKeys(keys []*vmPb.BatchKey) ([]*vmPb.BatchKey, error)
	// GetKeysWithLock get keys from cache, record this operation to read set with lock
	GetKeysWithLock(keys []*vmPb.BatchKey, lockerName string) ([]*vmPb.BatchKey, error)
	// Lock lock a name in a contract
	Lock(lockerName string) error
	...
	// AppendWriteSetIntoRelyCache put write set into relyCache
	AppendWriteSetIntoRelyCache(writeSet []*common.TxWrite)
	// AppendRelyCacheIntoRelyCache put relyCache into relyCache
	AppendRelyCacheIntoRelyCache(relyCache []map[string][]byte)
	// GetRelyCache get rely cache
	GetRelyCache() []map[string][]byte
	// PutRelyTxRWSetTable put txRWSet table into RelyTxRWSetTable
	PutRelyTxRWSetTable(relyTxRWSetTable []*common.TxRWSet)
