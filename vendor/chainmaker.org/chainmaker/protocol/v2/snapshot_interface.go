...
type Snapshot interface {
	...
	// GetKeyWithLock Read the key from the current snapshot and the previous snapshot with lock
	GetKeyWithLock(txExecSeq int, contractName, lockerName, txId string, key []byte) ([]byte, error)
	...
	// GetKeysWithLock Read the key from the current snapshot and the previous snapshot with lock
	GetKeysWithLock(txExecSeq int, keys []*vmPb.BatchKey, lockerName, txId string) ([]*vmPb.BatchKey, error)
	...
	ApplyWritesToWriteTable(writes []*common.TxWrite)
	...
	Lock(lockerName, txId string) error
}
