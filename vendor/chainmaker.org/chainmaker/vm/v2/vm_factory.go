...
func (m *VmManagerImpl) OnMessage(msg *msgbus.Message) {
	...
	case msgbus.ChainConfig:
		...
		if chainConfig.GetBlockVersion() < v2370 {
			return
		}
...
func (m *VmManagerImpl) processSupportList(chainConfig *config.ChainConfig) {
	newSupports := make(map[commonPb.RuntimeType]struct{})
	m.lock.Lock()
	defer m.lock.Unlock()
	...
	for _, vmConfig := range chainConfig.GetVm().GetSupportList() {
		...
		newSupports[runtimeType] = struct{}{}
		...
		if _, ok = m.InstanceManagers[runtimeType]; ok {
			...
			// there is a logical flaw in v237, omitted to add Go runtime type
			// when enable the existed Docker-Go runtime type
			if blockVersion != v2370 {
				m.processAddDockerGo(runtimeType, goEnabled, goExists, chainConfig.GetChainId())
			}
			continue
		}
	...
	for vmType := range m.aliveManagers {
		if _, ok = newSupports[vmType]; !ok {
			m.deleteRuntimeType(blockVersion, vmType, newSupports)
		}
	}
}

func (m *VmManagerImpl) processAddDockerGo(runtimeType commonPb.RuntimeType, goEnabled, goExists bool, chainId string) {
	if runtimeType == commonPb.RuntimeType_DOCKER_GO && !goEnabled {
		if goExists {
			m.aliveManagers[commonPb.RuntimeType_GO] = struct{}{}
			m.Log.Infof("enabled runtimeType %s", commonPb.RuntimeType_GO)
		} else {
			m.addNewRuntimeType(chainId, commonPb.RuntimeType_GO)
		}
	}
}

// deleteRuntimeType deletes runtime types not supported yet
func (m *VmManagerImpl) deleteRuntimeType(blockVersion uint32, delRuntimeType commonPb.RuntimeType,
	newSupports map[commonPb.RuntimeType]struct{}) {
	// there is a logical flaw in v237, disabled both Go and Docker-Go engine
	// when we want to delete Go or Docker-Go runtime type
	if blockVersion == v2370 {
		if delRuntimeType == commonPb.RuntimeType_DOCKER_GO || delRuntimeType == commonPb.RuntimeType_GO {
			delete(m.aliveManagers, commonPb.RuntimeType_DOCKER_GO)
			m.Log.Infof("disabled runtime type %q engine", commonPb.RuntimeType_DOCKER_GO)
			delete(m.aliveManagers, commonPb.RuntimeType_GO)
			m.Log.Infof("disabled runtime type %q engine", commonPb.RuntimeType_GO)
		} else {
			delete(m.aliveManagers, delRuntimeType)
			m.Log.Infof("disabled runtime type %q engine", delRuntimeType)
		}
		return
	}
	// above v238
	var ok bool
	// go engine maybe used by docker go runtime type, so we deal with it separately
	if delRuntimeType != commonPb.RuntimeType_GO {
		delete(m.aliveManagers, delRuntimeType)
		m.Log.Infof("disabled runtime type %q engine", delRuntimeType)
		// docker go runtime type maybe have used a go engine, so we deal with it separately
		if delRuntimeType != commonPb.RuntimeType_DOCKER_GO {
			return
		}
	}
	// only when neither Go nor Docker-Go is still supported, we can disable the Go engine
	supportGo := true
	supportDockerGo := true
	if delRuntimeType == commonPb.RuntimeType_GO {
		supportGo = false
		if _, ok = newSupports[commonPb.RuntimeType_DOCKER_GO]; !ok {
			supportDockerGo = false
		}
	} else {
		supportDockerGo = false
		if _, ok = newSupports[commonPb.RuntimeType_GO]; !ok {
			supportGo = false
		}
	}
	if !supportGo && !supportDockerGo {
		delete(m.aliveManagers, commonPb.RuntimeType_GO)
		m.Log.Infof("disabled runtime type %q engine", commonPb.RuntimeType_GO)
	}
}
