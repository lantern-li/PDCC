...
func NewTxSimContext(vmManager protocol.VmManager, snapshot protocol.Snapshot, tx *common.Transaction,
	...
	return &txSimContextImpl{
		...
		relyCache:                        make([]map[string][]byte, 0, defaultRelyCacheSize),
		relyTxRWSetTable:                 make([]*common.TxRWSet, 0),
		isFollower:                       false,
	}
}

// NewTxSimContextForFollower is used to create a new simContext for a transaction
func NewTxSimContextForFollower(vmManager protocol.VmManager, snapshot protocol.Snapshot, tx *common.Transaction,
	blockVersion uint32, logger protocol.Logger) protocol.TxSimContext {
	txRWSet := make([]map[string]*rwSet, protocol.CallContractDepth+1)
	txRWSet[0] = make(map[string]*rwSet)
...
type txSimContextImpl struct {
	...
	relyCache        []map[string][]byte // 使用slice是为了减少map中的hash计算而导致的时间开销
	relyTxRWSetTable []*common.TxRWSet
	isFollower       bool
}
...
// Lock a name in contract
func (s *txSimContextImpl) Lock(lockerName string) error {
	return s.snapshot.Lock(lockerName, s.tx.Payload.TxId)
}

// GetWithLock key from cache, record this operation to read set
func (s *txSimContextImpl) GetWithLock(contractName, lockerName string, key []byte) ([]byte, error) {
	...
	// 插入一个cache
	if value, done := s.getFromRelyCache(contractName, key); done {
	...
	var value []byte
	var err error
	if value, err = s.snapshot.GetKeyWithLock(s.txExecSeq, contractName, lockerName, s.tx.Payload.TxId, key); err != nil {
		return nil, err
	}
...
func (s *txSimContextImpl) GetKeys(keys []*vmPb.BatchKey) ([]*vmPb.BatchKey, error) {
	...
	var (
		...
		cacheValues       []*vmPb.BatchKey
		emptyCacheKeys    []*vmPb.BatchKey
	)
	...
	// 插入一个cache
	if cacheValues, emptyCacheKeys, done = s.getBatchFromRelyCache(emptyReadSetKeys); done {
		// if get from db success, put into read set
		s.PutBatchIntoReadSet(cacheValues)
		return append(cacheValues, append(writeSetValues, readSetValues...)...), nil
	}

	// batch get from db
	value, err := s.snapshot.GetKeys(s.txExecSeq, emptyCacheKeys)
	if err != nil {
		return nil, err
	}

	if len(cacheValues) != 0 {
		value = append(value, cacheValues...)
	}
...
func (s *txSimContextImpl) GetKeysWithLock(keys []*vmPb.BatchKey, lockerName string) ([]*vmPb.BatchKey, error) {
	// 从节点不走lock
	if s.isFollower {
	...
	// batch get from db
	value, err := s.snapshot.GetKeysWithLock(s.txExecSeq, emptyCacheKeys, lockerName, s.tx.Payload.TxId)
	if err != nil {
		return nil, err
	}
...
func (s *txSimContextImpl) getTxRWSetTable() []*common.TxRWSet {
	// 1. 从节点，但是未使用交易并行执行优化逻辑
	/**
		len(s.relyCache) == 0的几种情况：
		a. 从节点，但是未使用relyCache进行simulate并行加速
		b. 从节点，使用了并行加速，但是没有交易冲突
	上述情况，都可以直接返回snapshot维护的RWSetTable即可
	*/
	if len(s.relyCache) == 0 {
		return s.snapshot.GetTxRWSetTable()
	}

	// 3. 从节点，使用了交易并行执行优化逻辑
	return s.relyTxRWSetTable
}
...
func (s *txSimContextImpl) Select(contractName string, startKey []byte, limit []byte) (protocol.StateIterator, error) {
	...
	txRWSets := make([]*common.TxRWSet, 0)
	if s.isFollower {
		txRWSets = s.getTxRWSetTable()
	} else {
		// 主节点的迭代器交易会放回最后重新串行执行，因此可以使用原逻辑（使用snapshot维护的RWSetTable）
...
// GetRelyCache get rely cache
func (s *txSimContextImpl) GetRelyCache() []map[string][]byte {
	return s.relyCache
}

// AppendWriteSetIntoRelyCache put write set into rely cache.
func (s *txSimContextImpl) AppendWriteSetIntoRelyCache(writeSet []*common.TxWrite) {
	cache := make(map[string][]byte, len(writeSet))
	for _, writeData := range writeSet {
		key := constructKey(writeData.ContractName, writeData.Key)
		cache[key] = writeData.Value
	}
	s.relyCache = append(s.relyCache, cache)
}

// AppendRelyCacheIntoRelyCache put rely txs' rely cache into self rely cache
func (s *txSimContextImpl) AppendRelyCacheIntoRelyCache(relyCache []map[string][]byte) {
	s.relyCache = append(s.relyCache, relyCache...)
}

// PutRelyTxRWSetTable put rely txs' rely tx rwSet table
func (s *txSimContextImpl) PutRelyTxRWSetTable(relyTxRWSetTable []*common.TxRWSet) {
	s.relyTxRWSetTable = relyTxRWSetTable
}
...
// getFromRelyCache get from rely Cache
func (s *txSimContextImpl) getFromRelyCache(contractName string, key []byte) ([]byte, bool) {
	// 主节点走不到这
	if s.relyCache == nil || len(s.relyCache) == 0 {
		return nil, false
	}

	//check every depth
	finalKey := constructKey(contractName, key)
	for _, currentRelyCacheMap := range s.relyCache {
		val, ok := currentRelyCacheMap[finalKey]
		if !ok {
			continue
		}
...
// getBatchFromReadSet  fetch txs from read set
func (s *txSimContextImpl) getBatchFromRelyCache(keys []*vmPb.BatchKey) ([]*vmPb.BatchKey,
	[]*vmPb.BatchKey, bool) {
	// todo optimize logic
	txReads := make([]*vmPb.BatchKey, 0, len(keys))
	emptyTxReadsKeys := make([]*vmPb.BatchKey, 0, len(keys))
	...
	for _, key := range keys {
		//if has value, add txReads, else add emptyTxReadsKeys
		if value, ok := s.getFromRelyCache(key.ContractName, protocol.GetKeyStr(key.Key, key.Field)); ok {
			key.Value = value
			txReads = append(txReads, key)
		} else {
			emptyTxReadsKeys = append(emptyTxReadsKeys, key)
