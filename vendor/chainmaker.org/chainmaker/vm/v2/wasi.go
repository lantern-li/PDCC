...
// ContractEncrypt encrypt contract field
func (w *WacsiImpl) ContractEncrypt(requestBody []byte, kmsProvider protocol.KMSProvider, memory []byte,
	data []byte, isLen bool) ([]byte, error) {
	ec := serialize.NewEasyCodecWithBytes(requestBody)
	plainData, e1 := ec.GetBytes("plain_data")
	if e1 != nil {
		return nil, e1
	}
	...
	w.logger.Debugf("wacsiImpl::ContractEncrypt() ==> plainData = %v, ", plainData)
	if kmsProvider == nil {
		msg := fmt.Errorf("[contract encrypt] fail. kmsProvider is nil")
		return nil, msg
	}
	value, err := kmsProvider.Encrypt(plainData)
	if err != nil {
		msg := fmt.Errorf("[contract encrypt] fail. plainData=%v, error:%v", plainData, err.Error())
		return nil, msg
	}
	w.logger.Debugf("wacsiImpl::ContractEncrypt() ==> plainData encrypt result = %v \n", value)
	copy(memory[valuePtr:valuePtr+4], bytehelper.IntToBytes(int32(len(value))))
	if len(value) == 0 {
		return nil, nil
	}
...
// ContractDecrypt decrypt contract field
func (w *WacsiImpl) ContractDecrypt(requestBody []byte, kmsProvider protocol.KMSProvider, memory []byte,
	data []byte, isLen bool) ([]byte, error) {
	ec := serialize.NewEasyCodecWithBytes(requestBody)
	cipherData, e1 := ec.GetBytes("cipher_data")
	if e1 != nil {
		return nil, e1
	}
	valuePtr, e2 := ec.GetInt32("value_ptr")
	...
	w.logger.Debugf("wacsiImpl::ContractDecrypt() ==> cipherData = %v \n", cipherData)
	if kmsProvider == nil {
		msg := fmt.Errorf("[contract decrypt] fail. kmsProvider is nil")
		return nil, msg
	}
	value, err := kmsProvider.Decrypt(cipherData)
	if err != nil {
		msg := fmt.Errorf("[contract decrypt] fail. cipherData=%v, error:%v", cipherData, err.Error())
		return nil, msg
	}
	w.logger.Debugf("wacsiImpl::ContractDecrypt() ==> cipherData decrypt result = %v \n", value)
	copy(memory[valuePtr:valuePtr+4], bytehelper.IntToBytes(int32(len(value))))
	if len(value) == 0 {
		return nil, nil
	}
