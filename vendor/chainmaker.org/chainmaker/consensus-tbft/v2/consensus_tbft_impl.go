...
func New(config *consensusUtils.ConsensusImplConfig) (*ConsensusTBFTImpl, error) {
	...
	consensus.updateRemoteC = make(chan interface{}, defaultChanCap)
...
func (consensus *ConsensusTBFTImpl) Start() error {
	...
	go func() {
		...
		// update remote status by consensus messages
		go consensus.updateRemoteByConsensusMessages()
...
func (consensus *ConsensusTBFTImpl) handleProposedBlock(proposedProposal *proposedProposal) {
	...
	necessary, err := consensus.VerifySelfProposal(consensus.Proposal)
	if err != nil {
		consensus.logger.Errorf("verify self proposal shouldn't have err %s", err)
		return
	}
	if necessary {
		consensus.Proposal = NewTBFTProposal(consensus.Proposal.PbMsg, true)
	}
...
func (consensus *ConsensusTBFTImpl) handleVerifyResult(verifyResult *consensuspb.VerifyResult) {
	...
	if consensus.Step != tbftpb.Step_PROPOSE ||
		!bytes.Equal(consensus.VerifingProposal.PbMsg.Block.Header.BlockHash, hash) &&
			!bytes.Equal(consensus.VerifingProposal.PbMsg.Block.Header.BlockHash, []byte(verifyResult.Msg)) {
...
func (consensus *ConsensusTBFTImpl) procPropose(proposal *tbftpb.Proposal) {
	...
	// update remote status by the proposal
	if proposal.GetVoter() != consensus.Id {
		consensus.addConsensusMsgForUpdateRemote(proposal)
	}
	...
	if proposer != proposal.Voter {
		consensus.logger.Warnf("[%s](%d/%d/%s) proposer: %s, receive proposal from incorrect proposer: %s",
	...
	consensus.msgbus.PublishSafe(msgbus.VerifyBlockWithRWSet, proposal)
...
func (consensus *ConsensusTBFTImpl) procPrevote(prevote *tbftpb.Vote) {
	...
	// update remote status by the preVote
	if prevote.GetVoter() != consensus.Id {
		consensus.addConsensusMsgForUpdateRemote(prevote)
	}
...
func (consensus *ConsensusTBFTImpl) procPrecommit(precommit *tbftpb.Vote) {
	...
	// update remote status by the preCommit vote
	if precommit.GetVoter() != consensus.Id {
		consensus.addConsensusMsgForUpdateRemote(precommit)
	}
...
func (consensus *ConsensusTBFTImpl) enterPrecommit(height uint64, round int32) {
	...
	default:
		consensus.logger.Warnf("[%s](%v/%v/%v) generated precommit (%d/%d/%x), time[sign:%dms],"+
			...
			signPrecommitTime.Sub(enterPrecommitTime).Milliseconds())
		return
	}
...
func (consensus *ConsensusTBFTImpl) verifyProposal(proposal *tbftpb.Proposal) error {
	...
	if consensus.Height == proposal.Height && (consensus.Round > proposal.Round || consensus.Step != tbftpb.Step_PROPOSE) {
		return nil
	}
	...
	if err != nil {
		// 这个错误目前在确定性调度无法避免，还是偶尔发送滞后的proposal，导致区块验签不过。此处将日志级别改为WARN
		consensus.logger.Warnf("[%s](%d/%d/%s) receive proposal VerifyPolicy result: %v, error %v",
...
// updateRemoteByConsensus updates remote status by consensus messages
func (consensus *ConsensusTBFTImpl) updateRemoteByConsensus(
	nodeId string, proposal *tbftpb.Proposal, vote *tbftpb.Vote) {
	rs, needToUpdate := consensus.checkNeedToUpdateRemote(nodeId, proposal, vote)
	if !needToUpdate {
		return
	}
	rs.Lock()
	defer rs.Unlock()
	// update remote status by a received proposal from the remote node
	if proposal != nil {
		// double check
		if proposal.Height < rs.Height {
			return
		}
		rs.Height = proposal.Height
		rs.Round = proposal.Round
		rs.Step = tbftpb.Step_PROPOSE
		rs.Proposal = proposal.GetBlock().GetHeader().GetBlockHash()
		rs.VerifingProposal = rs.Proposal
		consensus.logger.DebugDynamic(func() string {
			return fmt.Sprintf("[%s](%d/%d/%s) update remote[%s] by proposal(%d/%d/%x)",
				consensus.Id, consensus.Height, consensus.Round, consensus.Step, nodeId,
				proposal.Height, proposal.Round, rs.Proposal)
		})
		return
	}
	// update remote status by a received vote from the remote node
	if vote != nil {
		// double check
		...
		switch vote.Type {
		case tbftpb.VoteType_VOTE_PREVOTE:
			preVotes = NewVoteSet(consensus.logger, tbftpb.VoteType_VOTE_PREVOTE, vote.Height, vote.Round, nil)
			added, err = preVotes.AddVoteForConsistent(vote)
			if !added || err != nil {
				consensus.logger.Errorf("updateRemoterByConsensus failed, AddVoteForConsistent error: %s", err)
				return
			}
			rs.Step = tbftpb.Step_PREVOTE
		case tbftpb.VoteType_VOTE_PRECOMMIT:
			preCommits = NewVoteSet(consensus.logger, tbftpb.VoteType_VOTE_PRECOMMIT, vote.Height, vote.Round, nil)
			added, err = preCommits.AddVoteForConsistent(vote)
			if !added || err != nil {
				consensus.logger.Errorf("updateRemoterByConsensus failed, AddVoteForConsistent error: %s", err)
				return
			}
			rs.Step = tbftpb.Step_PRECOMMIT
		}
		rs.RoundVoteSet = newRoundVoteSet(vote.Height, vote.Round, preVotes, preCommits)
		rs.Height = vote.Height
		rs.Round = vote.Round
		consensus.logger.DebugDynamic(func() string {
			return fmt.Sprintf("[%s](%d/%d/%s) update remote[%s] by vote(%d/%d/%x)",
				consensus.Id, consensus.Height, consensus.Round, consensus.Step, nodeId,
				vote.Height, vote.Round, vote.Hash)
		})
	}
}

// checkNeedToUpdateRemote checks the proposal or vote can trigger updating the remote status or not
func (consensus *ConsensusTBFTImpl) checkNeedToUpdateRemote(
	nodeId string, proposal *tbftpb.Proposal, vote *tbftpb.Vote) (*RemoteState, bool) {
	if proposal == nil && vote == nil {
		return nil, false
	}
	node, err := consensus.consistentEngine.GetNodeStatus(nodeId)
	if err != nil || node == nil {
		consensus.logger.Warnf("[%s](%d/%d/%s) GetNodeStatus failed, nodeId:%s, error: %+v",
			consensus.Id, consensus.Height, consensus.Round, consensus.Step, nodeId, err)
		return nil, false
	}
	var (
		ok    bool
		state consistent_service.Status
		rs    *RemoteState
	)
	state, ok = node.Statuses()[TypeRemoteTBFTState]
	if !ok {
		consensus.logger.Warnf("[%s](%d/%d/%s) state not exist, nodeId:%s",
			consensus.Id, consensus.Height, consensus.Round, consensus.Step, nodeId)
		return nil, false
	}
	rs, ok = state.(*RemoteState)
	if !ok || rs == nil {
		consensus.logger.Warnf("[%s](%d/%d/%s) updateRemoterByConsensus failed, nodeId:%s,"+
			"state is empty or not RemoteState",
			consensus.Id, consensus.Height, consensus.Round, consensus.Step, nodeId)
		return nil, false
	}
	rs.RLock()
	defer rs.RUnlock()
	// the received consensus information lags behind the state of this node
	// that we currently maintain, and therefore no update is required
	if proposal != nil && proposal.Height < rs.Height {
		return nil, false
	}
	if vote != nil && vote.Height < rs.Height {
		return nil, false
	}
	return rs, true
}

// addConsensusMsgForUpdateRemote try to pass a proposal or vote message
// to the goroutine that updating remote status by received consensus messages
func (consensus *ConsensusTBFTImpl) addConsensusMsgForUpdateRemote(msg interface{}) {
	select {
	case consensus.updateRemoteC <- msg:
	default:
	}
}

// updateRemoteByConsensusMessages is the goroutine for updating remote status by received consensus messages
func (consensus *ConsensusTBFTImpl) updateRemoteByConsensusMessages() {
	for {
		select {
		case <-consensus.closeC:
			consensus.logger.Infof("goroutine updateRemoteByConsensusMessages stopped")
			return
		case msg := <-consensus.updateRemoteC:
			if proposal, ok := msg.(*tbftpb.Proposal); ok {
				consensus.updateRemoteByConsensus(proposal.Voter, proposal, nil)
			} else if vote, ok := msg.(*tbftpb.Vote); ok {
				consensus.updateRemoteByConsensus(vote.Voter, nil, vote)
			}
		}
	}
}

func (consensus *ConsensusTBFTImpl) VerifySelfProposal(proposal *TBFTProposal) (bool, error) {
	bm := proposal.PbMsg
	result, err := consensus.blockVerifier.VerifyBlockSync(bm.Block, protocol.PROPOSER_VERIFY)
	if err != nil {
		return false, err
	}
	// verify result is nil means this step is unnecessary
	if result == nil {
		return false, nil
	}
	// calc block hash again, this time it is the final hash
	// note: don't need to sign block again
	hash, err := utils.CalcBlockHash(consensus.chainConf.ChainConfig().Crypto.Hash, result.VerifiedBlock)
	if err != nil {
		return false, err
	}

	// update proposal too
	bm.Block.Header.BlockHash = hash[:]
	return true, err
}
