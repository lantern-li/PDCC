...
func (r *RemoteState) Update(state consistent_service.Status) {
	...
	if !ok || remote.Id != r.Id {
		return
	}
	// the received consistent information lags behind the state of this node
	// that we currently maintain, and therefore no update is required.
	// If the new height is equal that we currently maintain, but the round is lags behind
	// (remote.Height == r.Height && remote.Round < r.Round), it is noted
	// that the remote node may be restarted, we should update it.
	if remote.Height < r.Height {
		return
	}
	r.Height = remote.Height
	r.Round = remote.Round
	r.Step = remote.Step
	r.Proposal = remote.Proposal
	r.ValidProposal = remote.ValidProposal
	r.RoundVoteSet = remote.RoundVoteSet
}
	...
	if voteSet.Prevotes != nil {
		for _, voter := range validators {
			...
			if vote != nil && tD.tbftImpl.Step < tbftpb.Step_PRECOMMIT &&
				...
				// avoid the lock of tbftImpl cause a deadlock
				select {
				case tD.tbftImpl.internalMsgC <- tbftMsg:
				default:
					tD.log.Warnf("received a prevote[%d:%d:%x] by consistent service,"+
	...
	if voteSet.Precommits != nil {
		for _, voter := range validators {
			...
			if vote != nil && tD.tbftImpl.Step < tbftpb.Step_COMMIT &&
				...
				case tD.tbftImpl.internalMsgC <- tbftMsg:
				default:
					tD.log.Warnf("received a precommit[%d:%d:%x] by consistent service,"+
						" but put failed, internalMsgC is full",
						vote.Height, vote.Round, vote.Hash)
				}
			}
