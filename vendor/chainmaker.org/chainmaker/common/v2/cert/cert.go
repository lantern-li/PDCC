...
import (
	...
	bcDilithium "chainmaker.org/chainmaker/common/v2/crypto/asym/dilithium"
...
// ParseCertificateFromCer - parse certification
func ParseCertificateFromCer(certFilePath string) (*x509.Certificate, error) {
	certRaw, err := ioutil.ReadFile(certFilePath)
	if err != nil {
		return nil, fmt.Errorf("read cert file [%s] failed, %s", certFilePath, err)
	}
	return ParseCertificateFromRaw(certRaw)
}

// ParseCertificateFromCer - parse certification
func ParseCertificateFromRaw(certRaw []byte) (*x509.Certificate, error) {
	bytes := make([]byte, base64.StdEncoding.DecodedLen(len(certRaw)))
	n, err := base64.StdEncoding.Decode(bytes, certRaw)
	if err != nil {
		return nil, err
	}

	cert, err := bcx509.ParseCertificate(bytes[:n])
	if err != nil {
		return nil, fmt.Errorf("ParseCertificate cert failed, %s", err)
	}
