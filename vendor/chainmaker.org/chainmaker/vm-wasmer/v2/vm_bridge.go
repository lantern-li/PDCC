...
func (s *WaciInstance) invoke(method interface{}) int32 {
	...
	case protocol.ContractMethodKvIterator:
		s.Sc.SpecialTxType = protocol.ExecOrderTxTypeSpecial
		return s.KvIterator()
	...
	// encrypt and decrypt
	case protocol.EncryptLen:
		return s.GetContractEncryptLen()
	case protocol.Encrypt:
		return s.GetContractEncrypt()
	case protocol.DecryptLen:
		return s.GetContractDecryptLen()
	case protocol.Decrypt:
		return s.GetContractDecrypt()
	default:
