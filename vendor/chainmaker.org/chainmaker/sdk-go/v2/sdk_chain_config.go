...
// CreateChainConfigScheduleUpdatePayload create chain config schedule update payload
func (cc *ChainClient) CreateChainConfigScheduleUpdatePayload(scheduleType, algorithmType uint32) (*common.Payload, error) {
	cc.logger.Debug("[SDK] begin to create [ScheduleUpdate] to be signed payload")

	var pairs []*common.KeyValuePair

	pairs = append(pairs, &common.KeyValuePair{
		Key:   utils.KeySchedulerType,
		Value: []byte(strconv.FormatUint(uint64(scheduleType), 10)),
	}, &common.KeyValuePair{
		Key:   utils.KeyAlgorithmType,
		Value: []byte(strconv.FormatUint(uint64(algorithmType), 10)),
	})
	...
	payload := cc.CreatePayload("", common.TxType_INVOKE_CONTRACT, syscontract.SystemContract_CHAIN_CONFIG.String(),
		syscontract.ChainConfigFunction_SCHEDULE_UPDATE.String(), pairs, seq+1, nil)
